        ...
        const response = await fetch(url, {
            ...
            headers: {
                ...
                ...(localStorage.getItem('access_token') || localStorage.getItem('authToken') || localStorage.getItem('token') ? {
                    'Authorization': 'Bearer ' + (localStorage.getItem('access_token') || localStorage.getItem('authToken') || localStorage.getItem('token'))
                } : {})
            }
        });
...
document.getElementById('register-btn').onclick = () => showPage('register');
// Ye function sections ko switch karta hai
function showPage(pageId) {
    // 1. Saare sections se 'active' class hatao
    const sections = document.querySelectorAll('.page-section');
    sections.forEach(section => {
        section.classList.remove('active');
    });

    // 2. Jo ID di gayi hai, us section ko 'active' karo
...
// Nav links (Login, Register) ko click-able banana
...
async function makeAuthenticatedRequest(url, options = {}) {
    ...
    // Get token from localStorage as backup - check both keys
